...
Attention 计算量近似 $O(L^2)$，padding 带来**无效计算**。在 LLM **Decode 阶段**（每个请求每次只生成 1 个 token），静态 batch 的问题更严重：
...
**只算新 token 的 Q/K/V，历史 K/V 从缓存读取**——Decode 阶段从 $O(L^2)$ 重复计算，降为每步 $O(L)$。
...
$$\text{KV\_size} = 2 \times L \times d_{\text{model}} \times \text{bytes\_per\_elem}$$

- 因子 2：K 和 V 各一份
- $L$：当前序列长度（输入 + 已生成长度）
- FP16 下 `bytes_per_elem = 2`

**完整模型**（$N$ 层、$H$ 个头，通常 $d_{\text{model}} = H \times d_k$）：

$$\text{Total KV} = 2 \times N \times L \times d_{\text{model}} \times \text{bytes}$$

**示例**：LLaMA-7B（$N=32$, $d_{\text{model}}=4096$），FP16，序列长度 $L=2048$：
...
$$\text{Total KV (batch)} = \sum_{i=1}^{B} \text{KV\_size}(L_i)$$
...
| **KV Cache** | 缓存历史 K/V，Decode 每步只算新 token，避免 $O(L^2)$ 重复 |
